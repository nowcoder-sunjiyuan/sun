...
2.  **杜绝缓冲区溢出（Buffer Overflow）**：
    *   **C 字符串**: 进行字符串拼接（如 `strcat`）时，如果目标数组空间不足，就会发生缓冲区溢出，导致数据被覆盖，甚至引发程序崩溃。
    *   **SDS**: 在进行修改操作时，其 API 会先检查内部记录的可用空间 `alloc` 是否足够。如果不够，SDS 会自动进行扩容，分配一块更大的内存空间，然后再执行修改。这从根本上杜绝了缓冲区溢出的风险。
